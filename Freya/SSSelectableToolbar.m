...
-(IBAction)toolbarItemClicked:sender
{
    ...
    if ([[((SSSelectableToolbarItem*)sender) label] isEqualToString:@"Download"]) {
        [d.databaseController willLoadView];
    }else if([[((SSSelectableToolbarItem*)sender) label] isEqualToString:@"Modify"]){
        [d.modifyController willLoadView];
    }
