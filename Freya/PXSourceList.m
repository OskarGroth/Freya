...
- (NSRect)frameOfCellAtColumn:(NSInteger)column row:(NSInteger)row
{
	...
	{	
		...
		//Allow space for an icon if need be
				...
				leftIndent += [self iconSize].width+(ICON_SPACING)*10;
	...
	{
		CGFloat leftIndent = [self levelForRow:row]*[self indentationPerLevel]+5;
		...
		if(![self isGroupItem:item]&&[_secondaryDataSource respondsToSelector:@selector(sourceList:itemHasIcon:)])
		{
			if([_secondaryDataSource sourceList:self itemHasIcon:item]) {
				leftIndent += [self iconSize].width+(ICON_SPACING*5);
			}
		}
			...
			if([_secondaryDataSource respondsToSelector:@selector(sourceList:iconForItem:)])
			{
				...
				if(icon!=nil)
				{
					...
					NSSize actualIconSize = [icon size];
                        ...
                        iconRect = NSMakeRect(NSMinX(cellFrame)-iconSize.width-ICON_SPACING-35,
                                              NSMidY(cellFrame)-(iconSize.height/1.6f),
                                              iconSize.width+38,
                                              iconSize.height+12);					
					}else {                        
                        ...
                        iconRect = NSMakeRect(NSMinX(cellFrame)-iconSize.width-ICON_SPACING-8,
                                                     NSMidY(cellFrame)-(iconSize.height/1.6f),
                                                     iconSize.width+5,
                                                     iconSize.height+5);
                    }
...
- (void)outlineView:(NSOutlineView *)outlineView willDisplayCell:(id)cell forTableColumn:(NSTableColumn *)tableColumn item:(id)item
{
    ...
    [((NSCell*)cell).controlView addSubview:k];
